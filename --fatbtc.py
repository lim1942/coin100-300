...
def parse(exchange_id,exchange_name=file_name):
    ...
    def get_symbols():
        ...
        url = 'https://fatbtc.com/m/allticker/1/1542959482000'
        res = json_download(url,proxies=SOCK_PROXIES,headers=HEADERS)
        # 2 config symbols result
        res = res['data'].values()
        symbols = []
        for i in res:
            #3 config symbols subject
            subject = i['dspName'].replace('/','^')
...
if __name__ == '__main__':
    ...
    exchange_id = '104'
    # parse(exchange_id)
    print(requests.get('https://fatbtc.com/m/allticker/1/1542959482000',headers=HEADERS,proxies=SOCK_PROXIES,verify=False).content)
